...
class TouchDetectionApp(QWidget):
    ...
    def __init__(self):
        ...
        self.timer.setInterval(int(1000 / self.config.get('target_fps', 30)))
    ...
    def init_audio_system(self):
        ...
        try:
            # Initialize pygame first if not already done
            if not pygame.get_init():
                pygame.init()

            # Initialize mixer with safe parameters
            pygame.mixer.pre_init(44100, -16, 2, 2048)
            pygame.mixer.init()

            # Verify and load sound files with absolute paths
            sound_dir = os.path.join(os.path.dirname(__file__), 'Sounds')
            alert_path = os.path.join(sound_dir, 'alert.wav')
            warning_path = os.path.join(sound_dir, 'warning.wav')

            if not os.path.exists(alert_path) or not os.path.exists(warning_path):
                raise FileNotFoundError("Sound files missing")

            self.alert_sound = pygame.mixer.Sound(alert_path)
            self.warning_sound = pygame.mixer.Sound(warning_path)

            # Set volume levels
            self.alert_sound.set_volume(0.8)
            self.warning_sound.set_volume(0.6)
            ...
            try:
                pygame.mixer.init()  # Minimal init for system sounds
            except:
                pass
    ...
    def play_alert(self, level):
        def _play():
            try:
                ...
                print(f"Error in async sound: {e}")
    ...
    def init_emergency_contacts(self):
        """Load trusted contacts from JSON config"""
        try:
            with open('configPrivateInfo/contactInfo.json') as f:
                contact_info = json.load(f)
                self.emergency_contacts = {
                    "email": contact_info.get("email", "abc@gmail.com"),
                    "phone": contact_info.get("phone", "+1234567890")
                }
        except Exception as e:
    ...
    def send_emergency_alert(self, zone):
        """Send SMS/email on high-risk incidents and save screenshot"""
        if zone in ['groin', 'buttocks', 'manual']:
            timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
            message = f"EMERGENCY: Inappropriate contact detected ({zone}) at {timestamp}"
            success = False

            # Capture clean screenshot (no overlay)
            if hasattr(self, 'cap') and self.cap and self.cap.isOpened():
                ret, frame = self.cap.read()
                if ret and frame is not None:
                    try:
                        os.makedirs("incidents", exist_ok=True)
                        screenshot_path = os.path.join("incidents", f"incident_{timestamp}.jpg")
                        cv2.imwrite(screenshot_path, frame)

                        from email.message import EmailMessage
                        import smtplib

                        with open("configPrivateInfo/emailconfig.json") as f:
                            email_conf = json.load(f)

                        msg = EmailMessage()
                        msg['Subject'] = "[ALERT] TouchAware Incident Notification"
                        ...
                        with smtplib.SMTP_SSL(email_conf["smtp_server"], email_conf["smtp_port"]) as smtp:
                            smtp.login(email_conf["smtp_user"], email_conf["smtp_pass"])
                            smtp.send_message(msg)

                        print(f"✅ Email sent to {email_conf['to']}")
                        success = True
            ...
            if success:
                ...
                print(f"⚠️ ALERT ATTEMPTED BUT EMAIL FAILED: {message}")
